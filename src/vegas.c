...
void vegas(double (*fxn)(double[], double wgt, void *), double *tgral,
  ...
  double *regn = vP->regn;
  int ndim = vP->ndim;
  unsigned long ncall = vP->ncall;
  int init = vP->init;
  int itmx = vP->itmx;
  int nprn = vP->nprn;
  void *addPar = vP->addPar;
  static int i, it, j, k, mds, nd, ndo, ng, npg, ia[MXDIM + 1], kg[MXDIM + 1];
  static double calls, dv2g, dxg, f, f2, f2b, fb, rc, ti, tsi, wgt, xjac, xn,
      xnd, xo;
